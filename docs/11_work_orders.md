...
# T1-A 追加要求（2026-08-24，随主假设更换下达）

背景见 `docs/21_t2_pre_t1mini_audit.md`。三条硬性追加，缺一不可：

1. **clean SR 跨度 ≥60 个百分点**，按 7 bins × 3 seeds 执行。
   T1-mini 用的 ≥35% 门槛只适用于冒烟阶段，**T1-A 不得沿用**。
2. **必须同时报告两个量**：族内（同架构沿 checkpoint）的能力效应斜率，
   与跨架构在**匹配 clean SR** 下的 OOD 离散。只报其一无法支撑新主假设 H1′。
3. **新增观测冗余对照组**：同一架构、同一训练数据，**只增删腕部视角**，
   用 checkpoint 选择把两组 clean SR 匹配到同一 bin。
   这是把「腕部相机 4.8 倍差异」从机制候选变成机制证据的最短路径。
   注意：公开表格里能力与腕部视角是**混杂的**（97.4% vs 94.9%），
   所以必须自己做匹配，不能引用公开数字当证据。

**回报格式追加**
```
族内斜率（每架构、每轴）: [架构, 轴, gamma_hat, 95% CI, M 点数]
匹配能力离散: [轴, clean SR 窗口, 参与模型数, OOD 最小/最大, 倍数]
观测冗余对照: [有腕部 clean/OOD, 无腕部 clean/OOD, clean 是否匹配, 差异倍数]
```
