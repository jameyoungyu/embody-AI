...
2. **时域依赖检验**：按任务时域 H 分层，检验拟合关系的形状参数是否随 H 单调变化。
   机制预测：若逐步可靠性复合，则 `SR ≈ p^H`，log 尺度斜率应随 H 增大。
...
| 曲率显著且改变排名，但不随 H 变化 | 仍可发，但机制故事减半，降级为方法学短文 |
| 曲率显著、改变排名、且随 H 单调变化 | **全部成立**，按 Full Experiment 推进并申请资源 |
